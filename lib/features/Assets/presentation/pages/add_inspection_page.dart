...
class _AddInspectionPageState extends ConsumerState<AddInspectionPage> {
        ...
        builder: (context, setDialogState) => AlertDialog(
          ...
          actions: [
            ...
            ElevatedButton(
              ...
              child: Text('Done', style: body(color: tWhite)),
            ),
          ],
        ),
      ),
  ...
  Future<void> _submitInspection(String status) async {
    ...
    try {
      final now = DateFormat('MM/dd/yyyy, HH:mm:ss').format(DateTime.now());
      ...
      final response = await repository.addAssetInspectionInstance(payload);
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        throw Exception('Failed to save inspection: ${response.statusCode}');
      }
    } catch (e) {
      dSnackBar(
        context,
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('Add Inspection', style: body(weight: FontWeight.w600, size: 15)),
        // backgroundColor: tPrimary,
        // foregroundColor: tWhite,
            ...
            child: Column(
              ...
              children: [
                ...
                if (selectedTemplates.isNotEmpty) ...[
                  ...
                  ...selectedTemplates.map((template) => Padding(
                        padding: const EdgeInsets.only(bottom: 4),
                        child: Text(
                          '• ${template.name}',
                          style: body(size: 14, color: Colors.grey),
                        ),
                      )),
                ],
            ...
            Expanded(
              child: ElevatedButton(
                onPressed: isSubmitting
                    ? null
                    : () => _submitInspection('PENDING'),
                style: ElevatedButton.styleFrom(
                  backgroundColor: Colors.orange,
                  foregroundColor: tWhite,
                  minimumSize: const Size(0, 48),
                ),
                child: Text('Save', style: body(color: tWhite)),
              ),
            ),
            const SizedBox(width: 8),
            Expanded(
              child: ElevatedButton(
                onPressed: isSubmitting
                    ? null
                    : () => _submitInspection('COMPLETED'),
                style: ElevatedButton.styleFrom(
                  backgroundColor: Colors.green,
                  foregroundColor: tWhite,
                  minimumSize: const Size(0, 48),
                ),
                child: Text('Submit', style: body(color: tWhite)),
              ),
            ),
          ],
        ),
