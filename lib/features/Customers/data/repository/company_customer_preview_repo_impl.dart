...
class CompanyCustomerDetailsPreviewService {
  final String _authToken =
      'your_auth_token'; // Replace with your token management

  // Base URLs for endpoints
  final String _companyCustomerEndpoint =
      '${ApiConfig.baseUrl}companycustomer/';
  ...
  Future<http.Response> getMandatoryFields(
    ...
    final url = Uri.parse(
        '${_companyCustomerEndpoint}getMandatoryFields/$name/$companyId');
    return http.get(url, headers: _getHeaders());
  }

  Future<http.Response> getShowFields(String name, String companyId) async {
    final url =
        Uri.parse('${_companyCustomerEndpoint}getShowFields/$name/$companyId');
    return http.get(url, headers: _getHeaders());
  }

  Future<http.Response> getAllMandatoryFields(String companyId) async {
    final url = Uri.parse(
        '${_companyCustomerEndpoint}getAllMandatoryFields/$companyId');
    return http.get(url, headers: _getHeaders());
  }

  Future<http.Response> getAllShowFields(String companyId) async {
    final url =
        Uri.parse('${_companyCustomerEndpoint}getAllShowFields/$companyId');
    return http.get(url, headers: _getHeaders());
  }
  ...
  Future<void> deleteFile(String id) async {
    ...
    final response =
        await http.post(url, headers: _getHeaders(), body: json.encode(id));

    if (response.statusCode != 200) {
      throw Exception(
          'Failed to delete file with status code ${response.statusCode}');
    }
  }
  ...
  Future<void> getCompanyCustomerFile(String assetId, String savePath) async {
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception(
          'Failed to get file with status code ${response.statusCode}');
    }
  }
  ...
  Future<http.Response> getAssetByCustomerId(
    ...
    final url =
        Uri.parse('${_assetEndpoint}getByCutomerId/$customerId/$pageNumber');
    return http.get(url, headers: _getHeaders());
  }

  Future<http.Response> getWorkOrderByCustomerId(String customerId) async {
    final url = Uri.parse(
        '${_workorderEndpoint}getWorkOrderListByCustomerId/$customerId');
    return http.get(url, headers: _getHeaders());
  }

  Future<http.Response> getRoleAndPermission(String id, String name) async {
    final url =
        Uri.parse('${_customerEndpoint}roleAndPermissionByName/get/$id/$name');
    return http.get(url, headers: _getHeaders());
  }
}
